...
namespace SPTAG {
	namespace SSDServing {
		namespace VectorSearch {
			...
			public:
				SearchDefault()
					: m_workspaces(128), m_skipped(0), m_reassigned(0)
				{
					m_tids = 0;
					m_replicaCount = 4;
				...
				void LoadVectorIdsSSDIndex(std::string vectorTranslateMap, std::string extraFullGraphFile)
				{
					...
					*/
					...
					// Note: no free! I don't know where to free, leave it for now
					rwlock = new std::shared_mutex[1000000000];
					splitRoute = new std::pair<SizeType, SizeType>[1000000000];
					m_postingSizes = new std::atomic_uint32_t[1000000000];
					// m_postingRadius = new std::atomic<float>[1000000000];
				...
				ErrorCode Split(const SizeType headID, int appendNum, std::string& appendPosting)
				{
					...
					int first = 0;
					std::pair<SizeType, SizeType> newHeadsID;
					...
					for (int k = 0; k < m_k; k++) {
						...
						newHeadVID = begin;
						setPair(newHeadsID, k+1, newHeadVID);
...
#pragma omp parallel for schedule(dynamic)
				...
				void calAvgPostingSize()
				{
					int total = 0;
					int deleted = 0;
					for (int i = 0; i < m_postingNum.load(); i++)
					{
